...
namespace Advanced2D
{
	...
	// Resets sprite transforms by setting the identity matrix.
	...
	void Engine::SavePrimaryRenderTarget()
	{
		...
		this->p_device->GetRenderTarget(0, &ppMainSurface);
		this->p_device->GetDepthStencilSurface(&ppMainDepthStencilSurface);
	}

	void Engine::RestorePrimaryRenderTarget()
	{
		// restore normal render target
		this->p_device->SetRenderTarget(0, ppMainSurface);
		this->p_device->SetDepthStencilSurface(ppMainDepthStencilSurface);
	}
	...
	void Engine::UpdateMouse()
	{
		...
		for (int n=0; n<4; n++) 
		{
			...
			{
				game_mouseButton(p_input->GetPosX(), p_input->GetPosY(), n);
			}
		}
        ...
        */
		...
		bool Engine::collisionBR(Sprite *sprite1, Sprite *sprite2) 
		{
			...
			Rect *ra = new Rect(
				sprite1->getX(),
				sprite1->getY(),
				sprite1->getX() + sprite1->getWidth() * sprite1->getScaleH(), 
				sprite1->getY() + sprite1->getHeight() * sprite1->getScaleV() );
			Rect *rb = new Rect(
				sprite2->getX(), 
				sprite2->getY(),
				sprite2->getX() + sprite2->getWidth() * sprite2->getScaleH(), 
				sprite2->getY() + sprite2->getHeight() * sprite2->getScaleV() );
